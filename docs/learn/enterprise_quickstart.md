---
title: ✨ 엔터프라이즈 빠른 시작
sidebar_label: ✨ 엔터프라이즈 빠른 시작
description: Deploy LiteLLM 엔터프라이즈, validate the LLM/MCP/Agent gateway end-to-end, enforce budgets, and enable key enterprise controls.
---
...
Use this guide if you are on an **엔터프라이즈 trial** to evaluate LiteLLM as a unified **LLM, MCP, and Agent gateway** with enterprise controls and budget enforcement.
...
- **Full feature catalog**: [엔터프라이즈](/docs/enterprise)
...
- **Postgres** — required for 관리자 UI, virtual keys, MCP/Agent registries, and budget tracking
- An LLM provider API key (OpenAI, Azure, Anthropic, etc.)
- Your **엔터프라이즈 license key**

### Deploy with Docker Compose

Follow the [Docker Compose tab](/docs/proxy/docker_quick_start) in the 시작하기 튜토리얼. Condensed steps:
...
### Verify 엔터프라이즈 Edition

Open `http://localhost:4000/` — Swagger should show **"엔터프라이즈 Edition"** in the description. See the [엔터프라이즈 license FAQ](/docs/enterprise#how-do-i-set-up-and-verify-an-enterprise-license).

Open the 관리자 UI at `http://localhost:4000/ui` and sign in with your master key.
...
Complete these steps in the 관리자 UI before starting the gateway tracks.
...
→ [Multi-tenant 아키텍처](/docs/proxy/multi_tenant_architecture) · [가상 키](/docs/proxy/virtual_keys)
...
1. **Confirm model** `gpt-5.5` (or your model) appears in `model_list` (config or 관리자 UI → 모델).
...
5. **Verify logs** — open **로그** tab; confirm key, team, model, latency, and spend appear.

6. **Verify team spend** — open **Teams** tab → select your team; confirm spend incremented toward `max_budget`.


→ [가상 키](/docs/proxy/virtual_keys)
→ [Gateway 빠른 시작](/docs/learn/gateway_quickstart)
...
1. **등록 MCP server** — 관리자 UI → **MCP Servers** → Add New MCP Server:
...
3. **List tools** — confirm tools appear in 관리자 UI under **MCP Servers → MCP Tools**.
...
6. **Verify logs** — **로그** tab shows MCP tool call with namespaced tool name and cost.

→ [MCP 개요](/docs/mcp) · [MCP Permission Management](/docs/mcp_control) · [Using your MCP](/docs/mcp_usage)
...
2. **등록 in 관리자 UI** — **Agents** tab → **Add Agent** → enter name and URL.
...
6. **Verify logs** — **로그** tab shows key, team, latency, and agent-attributed cost. Cost counts toward team/key spend from Section 0.

→ [Agent Gateway 개요](/docs/a2a) · [Invoking A2A Agents](/docs/a2a_invoking_agents) · [Agent Cost Tracking](/docs/a2a_cost_tracking)
...
4. Confirm key spend in 관리자 UI under **가상 키**.

→ [가상 키](/docs/proxy/virtual_keys) · [Docker 빠른 시작 — RPM test](/docs/proxy/docker_quick_start)
...
→ [Multi-tenant 아키텍처](/docs/proxy/multi_tenant_architecture)
...
Enable via `store_audit_logs: true` under litellm_settings of your `config.yml`. Delete a virtual key via API or UI, then check the **Audit 로그** tab.

→ [Audit 로그](/docs/proxy/multiple_admins)

### Team/key guardrails

1. **가드레일** → create a guardrail (secret detection or content moderation)
...
→ [가드레일 빠른 시작](/docs/proxy/guardrails/quick_start)

### SSO for 관리자 UI

SSO controls **관리자 UI login** — separate from API auth (virtual keys or JWT). 등록 this redirect URI in your IdP:
...
Every 엔터프라이즈 license includes a dedicated Slack or Teams channel with our engineering team. Reach out to us `support@berri.ai` and we'll be more than happy to help you!
