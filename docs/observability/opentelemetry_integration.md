...
For **LiteLLM Proxy** setup, ingest token patterns, and trace verification, see **[Splunk 관측성 Cloud (OpenTelemetry)](/docs/observability/splunk_observability_cloud)**.
...
- Move large payloads off-trace (set `litellm.turn_off_message_logging=true` and rely on Spend 로그 / cold storage, referenced via `metadata.cold_storage_object_key`).
...
| `OTEL_LOGS_EXPORTER` | none | 로그 exporter (e.g. `console`) when events are enabled |
...
| Attribute | Value |
|---|---|
| `litellm.call_id` | Unique per `litellm.completion` invocation. Use this to correlate trace data with LiteLLM Spend 로그 and the LiteLLM UI |
...
| `metadata.spend_logs_metadata` | Custom metadata persisted to Spend 로그 |
