...
| service_callbacks | array of strings | System health monitoring - 로그 redis, postgres failures on specified services (e.g. datadog, prometheus) [Doc Metrics](prometheus) |
...
| master_key | string | The master key for the proxy [Set up 가상 키](virtual_keys) |
| database_url | string | The URL for the database connection [Set up 가상 키](virtual_keys) |
...
| public_routes | List[str] | (엔터프라이즈 Feature) Control list of public routes |
...
| alert_type_config | dict | 설정 mapping alert types to their handler settings |
...
| Name | Type | Description |
|------|------|-------------|
| routing_strategy | string | The strategy used for routing requests. Options: "simple-shuffle", "least-busy", "usage-based-routing", "latency-based-routing". Default is "simple-shuffle". [더 보기 information here](../routing) |
...
| enable_pre_call_check | boolean | If true, checks if a call is within the model's context window before making the call. [더 보기 information here](reliability) |
| content_policy_fallbacks | array of objects | Specifies fallback models for content policy violations. [더 보기 information here](reliability) |
| fallbacks | array of objects | Specifies fallback models for all types of errors. [더 보기 information here](reliability) |
...
| disable_cooldowns | boolean | If true, disables cooldowns for all models. [더 보기 information here](reliability) |
| retry_policy | object | Specifies the number of retries for different types of exceptions. [더 보기 information here](reliability) |
...
| alerting_config | AlertingConfig | [SDK-only arg] Slack alerting configuration. Defaults to None. [Further 문서](../routing.md#alerting-) |
...
| provider_budget_config | ProviderBudgetConfig | Provider budget configuration. Use this to set llm_provider budget limits. example $100/day to OpenAI, $100/day to Azure, etc. Defaults to None. [Further 문서](./provider_budget_routing.md) |
| enable_pre_call_checks | boolean | If true, checks if a call is within the model's context window before making the call. **Required** for `model_info.max_input_tokens` enforcement. Default: false. [더 보기 information here](reliability) |
...
| router_general_settings | RouterGeneralSettings | [SDK-Only] Router general settings - contains optimizations like 'async_only_mode'. [문서](../routing.md#router-general-settings) |
...
| model_group_affinity_config | Dict[str, List[str]] | Per-model-group affinity flags. Keys are model group names; values are lists of checks to enable (`deployment_affinity`, `responses_api_deployment_check`, `session_affinity`). Groups not listed fall back to the global `optional_pre_call_checks`. [문서](../response_api.md#per-model-group-affinity-configuration) |
| ignore_invalid_deployments | boolean | If true, ignores invalid deployments. Default for proxy is True - to prevent invalid models from blocking other models from being loaded. |
| search_tools | List[SearchToolTypedDict] | List of search tool configurations for Search API integration. Each tool specifies a search_tool_name and litellm_params with search_provider, api_key, api_base, etc. [Further 문서](../search/index.md) |
| guardrail_list | List[GuardrailTypedDict] | List of guardrail configurations for guardrail load balancing. Enables load balancing across multiple guardrail deployments with the same guardrail_name. [Further 문서](./guardrails/guardrail_load_balancing.md) |
...
| AZURE_STORAGE_ACCOUNT_KEY | The Azure Storage Account Key to use for 인증 to Azure Blob Storage logging
...
| AZURE_STORAGE_TENANT_ID | The Application Tenant ID to use for 인증 to Azure Blob Storage logging
| AZURE_STORAGE_CLIENT_ID | The Application Client ID to use for 인증 to Azure Blob Storage logging
| AZURE_STORAGE_CLIENT_SECRET | The Application Client Secret to use for 인증 to Azure Blob Storage logging
...
| DYNAMOAI_API_KEY | API key for DynamoAI 가드레일 service
...
| DD_LLMOBS_ML_APP | Default ml_app name for Datadog LLM 관측성 (Application column). Falls back to DD_SERVICE. Can be overridden per-request via `metadata.ml_app`.
...
| LITELLM_MCP_STDIO_EXTRA_COMMANDS | Comma-separated extra command basenames allowed for MCP stdio transport beyond the built-in allowlist. 예제: `my-mcp-bin`. Empty by default
...
| GCS_PUBSUB_TOPIC_ID | PubSub Topic ID to send LiteLLM Spend로그 to.
| GCS_PUBSUB_PROJECT_ID | PubSub Project ID to send LiteLLM Spend로그 to.
...
| GENERIC_ROLE_MAPPINGS_ROLES | Python dict string mapping LiteLLM roles to SSO group names. 예제: `{"proxy_admin": ["admin-group"], "internal_user": ["users"]}`
...
| GITHUB_COPILOT_API_BASE | Base URL for GitHub Copilot API. For GitHub 엔터프라이즈 subscriptions with custom host, it is similar to https://copilot-api.my-company.ghe.com. Default is https://api.githubcopilot.com
| GITHUB_COPILOT_DEVICE_CODE_URL | URL for GitHub Copilot device code authentication. For GitHub 엔터프라이즈 subscriptions with custom host, it is similar to https://my-company.ghe.com/login/device/code. Default is https://github.com/login/device/code
| GITHUB_COPILOT_ACCESS_TOKEN_URL | URL for GitHub Copilot access token retrieval. For GitHub 엔터프라이즈 subscriptions with custom host, it is similar to https://my-company.ghe.com/login/oauth/access_token. Default is https://github.com/login/oauth/access_token
| GITHUB_COPILOT_API_KEY_URL | URL for GitHub Copilot API key retrieval. For GitHub 엔터프라이즈 subscriptions with custom host, it is similar to https://my-company.ghe.com/api/v3/copilot_internal/v2/token. Default is https://api.github.com/copilot_internal/v2/token
...
| GUARDRAILS_AI_API_BASE | Base URL for 가드레일 AI API
...
| HIDDENLAYER_AUTH_URL | 인증 URL for HiddenLayer. Defaults to `https://auth.hiddenlayer.ai`
...
| IBM_GUARDRAILS_API_BASE | Base URL for IBM 가드레일 API
| IBM_GUARDRAILS_AUTH_TOKEN | Authorization bearer token for IBM 가드레일 API
...
| LITELLM_UI_PATH | Path to directory for 관리자 UI files. Used when running with read-only filesystem (e.g., Kubernetes). Default is `/var/lib/litellm/ui` in Docker.
...
| MAXIMUM_TRACEBACK_LINES_TO_LOG | Maximum number of lines to log in traceback in LiteLLM 로그 UI. Default is 100
...
| MICROSOFT_SERVICE_PRINCIPAL_ID | Service Principal ID for Microsoft 엔터프라이즈 Application. (This is an advanced feature if you want litellm to auto-assign members to Litellm Teams based on their Microsoft Entra ID Groups)
...
| PILLAR_API_BASE | Base URL for Pillar API 가드레일
...
| SPEND_LOG_CLEANUP_BATCH_SIZE | Number of logs deleted per batch during cleanup. Default is 1000
| SPEND_LOG_PARTITION_INTERVAL | Granularity of LiteLLM_Spend로그 partitions when the table is partitioned: day, week, or month. Default is day
