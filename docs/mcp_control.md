...
- **Centralized control**: Manage all MCP permissions from the LiteLLM 관리자 UI or API
...
:::info 관련 문서
- [MCP 개요](./mcp.md) - 학습 about MCP in LiteLLM
- [MCP Cost Tracking](./mcp_cost.md) - Track costs for MCP tool calls
- [MCP 가드레일](./mcp_guardrail.md) - Apply security guardrails to MCP calls
- [Using MCP](./mcp_usage.md) - 사용 방법 MCP with LiteLLM
:::
...
1. Open **MCP Servers → Add / Edit** in the 관리자 UI.
...
- **Simplified 설정**: Use URLs instead of headers for server selection
...
- **Simplified 설정**: Easier setup for MCP clients that prefer URL-based configuration
...
| **설정** | Requires additional header | Self-contained in URL |
