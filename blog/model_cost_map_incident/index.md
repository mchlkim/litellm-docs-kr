...
`model_prices_and_context_window.json`의 잘못된 JSON 항목이 `main`에 merge되었습니다([`562f0a0`](https://github.com/BerriAI/litellm/commit/562f0a028251750e3d75386bee0e630d9796d0df)). 이로 인해 LiteLLM이 오래된 모델 비용 매핑 local copy로 조용히 fallback했습니다. 오래된 패키지 버전을 사용하는 사용자는 newer model(예: `azure/gpt-5.2`)에 대해서만 비용 추적을 잃었습니다. LLM 호출은 차단되지 않았습니다.
