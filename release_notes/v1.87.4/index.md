...
`v1.87.4` 는 다음 버전을 기반으로 한 패치 릴리스입니다: [`v1.87.3`](/release_notes/v1.87.3/v1-87-3). 이 릴리스는 다음 수정 사항을 백포트합니다: guardrail correctness fixes (a single pre-call hook for model-level guardrails, no DB re-init on every poll, 400 instead of 500 때 AIM blocks a request), caps Anthropic cache-control injection, 및 resolves several logging issues across Datadog batching, passthrough duplication, 및 Claude Code traces.
