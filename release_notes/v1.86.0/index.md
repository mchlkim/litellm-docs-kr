...
- **Native web-search citations for Anthropic clients** — LiteLLM now emits native `web_search_tool_result` blocks 따라서 Claude Desktop / Cowork render web-search citations correctly.
- **OTel-standard server-span attributes** — the proxy SERVER span now carries `http.response.status_code`, `http.route`, `url.path`, 및 `litellm.preprocessing.duration_ms`, 추가로 an opt-in for the experimental OTEL GenAI semantic conventions.
- **Componentized deployment** — additive scaffold + Helm chart to split the monolithic proxy into independently scalable `gateway`, `backend`, 및 `ui` services.
...
We expanded the set of Claude Code features that LiteLLM automatically tests against daily, 및 added a Known Issues section to the [Claude Code compatibility doc](https://docs.litellm.ai/docs/claude_code_compatibility) 따라서 customers can see which combinations are red, 및 why, 전에 hitting them in production.

This is a direct response to customer feedback on stability 및 regressions. The matrix is backed by a rigorous end-to-end suite that hits real provider endpoints with no mocking. The suite re-runs every day 및 the doc renders the latest LiteLLM stable against the latest Claude Code version.

<Image img={require('../../img/release_notes/claude_code_compat_matrix.png')} style={{ width: '800px', height: 'auto' }} />

Today's coverage sits at 76% across Anthropic, Bedrock Invoke, Bedrock Converse, Vertex AI, 및 Azure Foundry. Over the next week we plan to bring this to 90%. Coming soon, the same suite will gate PRs: any cell flipping green to red will fail the check 및 block merges into staging, making it much harder for code that breaks Claude Code to land in the next release.
...
Each Azure AI GPT-5.4 model also ships a dated snapshot alias (`gpt-5.4-2026-03-05`, `gpt-5.4-pro-2026-03-05`, `gpt-5.4-mini-2026-03-17`, `gpt-5.4-nano-2026-03-17`) — 9 catalog entries total. All GPT-5.4 entries include tiered (`>272k`) 및 priority pricing.

#### 기능

- **[Azure AI](https://docs.litellm.ai/docs/providers/azure_ai)**
    - Azure AI Foundry GPT-5.4 model metadata (gpt-5.4 / pro / mini / nano + dated aliases) 추가 - [PR #28030](https://github.com/BerriAI/litellm/pull/28030)
- **[Bedrock](https://docs.litellm.ai/docs/providers/bedrock)**
    - `jp.` cross-region inference profile for `claude-sonnet-4-6` 추가 - [PR #27976](https://github.com/BerriAI/litellm/pull/27976)
...
- **Anthropic Messages API (`/v1/messages`)**
    - native `web_search_tool_result` blocks for Anthropic clients (Claude Desktop / Cowork citations) emit - [PR #27886](https://github.com/BerriAI/litellm/pull/27886)
- **[Vector Stores](https://docs.litellm.ai/docs/vector_stores)**
    - vector store retrieve/list/update/delete 때 no completion model is set; merge URL query params into request data on those routes 수정 - [PR #27929](https://github.com/BerriAI/litellm/pull/27929)

#### 버그

- **[Batch API](https://docs.litellm.ai/docs/batches)**
    - Managed batches: convert raw provider `output_file_id` to managed ID in the `CheckBatchCost` poller 따라서 `GET /files/{id}/content` resolves routing - [PR #27984](https://github.com/BerriAI/litellm/pull/27984)
...
- **Auth / OAuth**
    - allowlisted redirect URIs in OAuth setup 허용 - [PR #27761](https://github.com/BerriAI/litellm/pull/27761)
- **Config**
...
- **[Lasso](https://docs.litellm.ai/docs/proxy/guardrails/quick_start)**
    - tool-calling support to LassoGuardrail (expands `tool_calls` / `role=tool` into Lasso `tool_use` / `tool_result` blocks; maps tool definitions) 추가 - [PR #27648](https://github.com/BerriAI/litellm/pull/27648)
...
- **Budgets** — Tighten budget field validation 및 add missing authorization checks on user self-update / key-generation paths - [PR #27897](https://github.com/BerriAI/litellm/pull/27897)
...
- `delegate_auth_to_upstream` in the `GET /v1/mcp/server` list API (`_build_mcp_server_table` was dropping it, 따라서 the dashboard always showed `false`) 노출 - [PR #27936](https://github.com/BerriAI/litellm/pull/27936)
...
- **Chat-completions fast path** — cache callback capabilities once instead of re-scanning `litellm.callbacks` per request; skip streaming-iterator wrapping 때 no callback needs it - [PR #27858](https://github.com/BerriAI/litellm/pull/27858)
- **Componentized deployment** — additive `gateway/`, `backend/`, `ui/` Dockerfiles + Helm chart (per-component 배포/Service/HPA, no edits to existing modules) - [PR #27557](https://github.com/BerriAI/litellm/pull/27557)
...
- Modernize model references in CI tests 및 configs - [PR #27856](https://github.com/BerriAI/litellm/pull/27856)
...
- unused GitHub Actions workflows 및 orphan files 제거 - [PR #27957](https://github.com/BerriAI/litellm/pull/27957)
- global Button/Tooltip mocks in per-file `@tremor/react` `vi.mock` (UI tests) 보존 - [PR #27958](https://github.com/BerriAI/litellm/pull/27958)
- Isolate `run_server` CLI tests from the Prisma DB-setup path - [PR #28029](https://github.com/BerriAI/litellm/pull/28029)
- response fields against the Interaction schema 검증 - [PR #28037](https://github.com/BerriAI/litellm/pull/28037)
...
**Full 변경 이력**: https://github.com/BerriAI/litellm/compare/v1.85.0...v1.86.0
...
* AI Integrations (Logging / 가드레일 / Secret Managers): 6
* 비용 추적, Budgets 및 Rate Limiting: 3
* MCP Gateway: 3
* 성능 / 부하 분산 / 안정성 improvements: 4
* 일반 Proxy Improvements (testing / CI / build): 12
* 문서 업데이트: 0
