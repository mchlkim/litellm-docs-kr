...
`v1.85.4` 는 다음 버전을 기반으로 한 패치 릴리스입니다: [`v1.85.3`](/release_notes/v1.85.3/v1-85-3). 이 릴리스는 다음 수정 사항을 백포트합니다: the same six staged fixes shipped on the 1.84 line: Azure AD token refresh, batch 및 video model routing, org-scoped team key creation, Vertex Claude effort handling, 및 duplicate passthrough cost callbacks.
